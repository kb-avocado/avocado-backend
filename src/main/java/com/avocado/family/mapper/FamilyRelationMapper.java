package com.avocado.family.mapper;
...
public interface FamilyRelationMapper {
    ...
    boolean existsActiveByChildId(@Param("childId") Long childId);

    // 아이가 새 요청을 만들 때, 진행 중이던 이전 요청을 모두 정리한다.
    int cancelInProgressByChildId(@Param("childId") Long childId);
    ...
    void insertRequest(FamilyRelation relation);

    // 폴링·보호자 확인·당사자 검증에 공통으로 쓰는 조회. 보호자와 아이 이름까지 채워 준다.
    FamilyRelation selectDetailById(@Param("id") Long id);
